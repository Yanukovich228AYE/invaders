...
// I didn't want to make PLAYER_SIZE, ENEMY_SIZE, BULLET_SIZE, so I just defined the base size.
constexpr unsigned char BASE_SIZE = 16;
constexpr unsigned char ENEMY_BULLET_SPEED = 2;
// I explained this in Enemy.hpp
...
// why not 64?. it's because if we set it to 0, they will still have 1 frame pause. So 63 + 1 = 64.
...
struct Bullet {
	...
	// We also use this struct in the Player class. But the player's bullets don't have a tail
...
struct Powerup {
	...
	// 1 - Fast reload
	// 2 - 3 bullets
	// 3 - Mirrored controls (power-down)
	// I should've used enums
	unsigned char type;
