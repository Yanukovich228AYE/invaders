	...
	shoot_distribution(0, ENEMY_SHOOT_CHANCE)
{
	...
	for (unsigned char a = 0; a < ENEMY_TYPES; a++)
		enemy_animations.push_back(Animation(1 + move_pause, BASE_SIZE, "Enemy" + std::to_string(static_cast<unsigned short>(a)) + ".png"));
}
...
void EnemyManager::reset(unsigned short i_level) {
	...
	switch (i_level)
	{
		...
		case 4:
			...
			break;
		case 5:
			// easy enemies at the top and hard in the bottom
			level_sketch = "0000000000000000\n2222222222222222\n1111111111111111\n1111111111111111";
			break;
		case 6:
	...
	// convert characters into enemies
...
void EnemyManager::update(std::mt19937_64& i_random_engine) {
	...
	// lambda to see if how many enemies are dead
	dead_enemies_start = remove_if(enemies.begin(), enemies.end(), [](const Enemy& i_enemy) {
		return i_enemy.get_health() == 0;
	});

	// The more enemies get killed the faster they become
	...
	// another lambda to erase enemy bullets so that they dont just fly outside the users map
